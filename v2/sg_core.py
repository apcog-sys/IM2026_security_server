...
# --------------------------------------------------------------------------
# Every gateway publishes this by default:
#     <instance>/<owner_class>/<owner_code>/sg-selftest/sg_test
# backed by its own /api/sg_test endpoint. Another gateway can discover it in
# the catalog, subscribe, and consume it - which exercises routing, sender
# authentication, signing, timestamping and the subscription gate end to end,
# proving SG <-> SG communication works.
SELFTEST_APP = "sg-selftest"
SELFTEST_SERVICE = "sg_test"


def selftest_service_id():
    return (f"{config.INSTANCE}/{config.OWNER_CLASS}/{config.OWNER_CODE}/"
            f"{SELFTEST_APP}/{SELFTEST_SERVICE}")


def provision_selftest():
    """Register the default self-test application + service (idempotent)."""
    add_application(config.OWNER_CLASS, config.OWNER_CODE, SELFTEST_APP, config.OWNER_NAME)
    # The gateway calls its own admin port for this backend; the admin container
    # serves /api/sg_test and shares the pod's network namespace.
    backend = f"http://127.0.0.1:{config.PORT}/api/sg_test"
    out = add_service(config.OWNER_CLASS, config.OWNER_CODE, SELFTEST_APP,
                      SELFTEST_SERVICE, backend,
                      f"Built-in connectivity self-test of {config.GATEWAY_CODE}")
    return {"ok": True, "service_id": selftest_service_id(), "backend": backend, **out}
...
SELFTEST_APP = "sg-selftest"       # application code owned by the gateway itself
SELFTEST_SERVICE = "sg_test"       # service code -> backed by this gateway's /api/sg_test
...
def provision_selftest():
    """Register this gateway's built-in self-test service (idempotent).

    Every gateway publishes <instance>/<owner>/sg-selftest/sg_test, backed by
    its own /api/sg_test. Any OTHER gateway can then discover it in the synced
    catalog, subscribe, and consume it — proving the whole SG<->SG path works
    (routing, sender auth, signing, timestamping, subscription checks).
    """
    add_application(config.OWNER_CLASS, config.OWNER_CODE, SELFTEST_APP,
                    config.OWNER_NAME)
    # The gateway calls its own admin port for this service (same pod/process).
    backend = f"http://127.0.0.1:{config.PORT}/api/sg_test"
    out = add_service(config.OWNER_CLASS, config.OWNER_CODE, SELFTEST_APP,
                      SELFTEST_SERVICE, backend,
                      f"Built-in connectivity self-test of {config.GATEWAY_CODE}")
    return {"ok": True, "service_id": selftest_service_id(), "backend": backend,
            "registered": out}
