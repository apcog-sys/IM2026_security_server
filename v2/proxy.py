...
from security_gateway import config, mesh, messagelog, models, sg_core
...
def subscribe(application_id_str: str, service_id_str: str) -> dict:
    ...
    try:
        r = mesh.post(f"{provider_gw['address']}/api/provider/subscribe",
...
def _prepare(consumer_id_str: str, service_id_str: str, body_obj,
    ...
    canonical = xroad.canonical_message(message_id, str(consumer), str(service),
                                        body_bytes, method)
    ...
    envelope = {
        ...
        "method": method, "body": body_obj, "signature": signature,
        "sign_cert_pem": hosted.sign_cert_pem,
    ...
    return {"message_id": message_id, "consumer": str(consumer), "service": str(service),
            "method": method, "provider_gw": provider_gw, "envelope": envelope,
...
def send(consumer_id_str: str, service_id_str: str, body_obj,
         method: str = "POST") -> dict:
    """Synchronous mediation: build + deliver + wait for the response."""
    p = _prepare(consumer_id_str, service_id_str, body_obj, method)
    ...
    try:
        resp = mesh.post(f"{p['provider_gw']['address']}/api/provider/receive",
...
def receive(env: dict) -> dict:
    ...
    method = (env.get("method") or "POST").upper()
    ...
    body_bytes = canonical_json(body_obj)
    canonical = xroad.canonical_message(message_id, consumer_str, service_str,
                                        body_bytes, method)
    ...
    # Invoke the backend with the METHOD the consumer signed for. Methods that
    # carry no body (GET/DELETE/HEAD) send the body dict as query parameters.
    try:
        if method in xroad.BODY_METHODS:
            backend = httpx.request(method, svc.backend_url, json=body_obj, timeout=30)
        else:
            backend = httpx.request(method, svc.backend_url,
                                    params=body_obj if isinstance(body_obj, dict) else None,
                                    timeout=30)
    ...
    messagelog.log(message_id, "in", consumer_str, service_str, imprint, signature,
                   ...
                   note=f"subscription ok; {method} {svc.backend_url}")
